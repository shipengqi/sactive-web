  ...
  use($$injector) {
    ...
    $$injector.getInstance('$$route')[self.method](self.path, async (ctx, next) => {
      ...
      if (!validateResult.status) {
        let e = new Error(validateResult.msg);
        e.status = 400;
        return self.$$config.errorHandler(e, ctx, next);
      }
      try {
        let response = await self.handler.call(self, ctx, next);
        if (response) {
          self.$$config.responseHandler(response, ctx, next);
        }
      } catch (e) {
        self.$$config.errorHandler(e, ctx, next);
      }
    });
  }
