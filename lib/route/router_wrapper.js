...
const {errors} = require('../response_formatter');
  ...
  use($$injector) {
    ...
    $$injector.getInstance('$$route')[self.method](self.path, async (ctx, next) => {
      ...
      if (!validateResult.status) {
        return self.$$config.responseTransform(new errors.ValidateError(validateResult.msg), null, ctx, next);
      }
      try {
        let response = await self.handler.call(self, ctx, next);
        if (response) {
          self.$$config.responseTransform(null, response, ctx, next, self.template);
        }
      } catch (e) {
        self.$$config.responseTransform(e, null, ctx, next);
      }
    });
  }
